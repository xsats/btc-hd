...
function random(len: number): Buffer {
  return toBuffer(CryptoJs.lib.WordArray.random(len) as any);
}
...
export function mnemonicToSeedSync(
  ...
  return toBuffer(
    PBKDF2(toWordArray(mnemonicBuffer), toWordArray(saltBuffer), {
      keySize: 512 / 32,
      iterations: 2048,
...
export function mnemonicToSeed(
  ...
  return new Promise((resolve, reject): void => {
    try {
      ...
      const res = toBuffer(
        CryptoJs.PBKDF2(toWordArray(mnemonicBuffer), toWordArray(saltBuffer), {
          keySize: 512 / 32,
          iterations: 2048,
          hasher: CryptoJs.algo.SHA512,
        })
